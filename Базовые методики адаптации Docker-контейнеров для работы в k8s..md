...
- [Базовые методики адаптации Docker-контейнеров для работы в k8s.](#---------------------------docker--------------------------k8s)
  * [1. Адаптация контейнерного образа (Dockerfile-level)](#1---------------------------------dockerfile-level-)
    + [1.1. Один процесс — один контейнер](#11------------------------------)
    + [1.2. Корректная работа PID 1](#12-------------------pid-1)
    + [1.3. Отказ от root-пользователя](#13----------root-------------)
    + [1.4 Для надежного развертывания и отката в Kubernetes используется концепция неизменяемых развертываний.](#14----------------------------------------kubernetes--------------------------------------------------)
  * [2. Конфигурация и переменные окружения](#2------------------------------------)
    + [2.1. Декларативная конфигурация](#21---------------------------)
    + [2.2. Использование ConfigMap и Secret](#22---------------configmap---secret)
  * [3. Работа с файловой системой и данными](#3-------------------------------------)
    + [3.1. Эфемерность контейнера](#31-----------------------)
    + [3.2. Volume-ориентированная архитектура](#32-volume----------------------------)
  * [4. Сетевое взаимодействие](#4-----------------------)
    + [4.1. Отказ от жёсткой привязки к IP](#41-----------------------------ip)
    + [4.2. Один порт — один сервис](#42------------------------)
  * [5. Health-checks и жизненный цикл](#5-health-checks-----------------)
    + [5.1. Readiness и Liveness probes](#51-readiness---liveness-probes)
    + [5.2. Startup-поведение](#52-startup----------)
  * [6. Управление ресурсами](#6---------------------)
    + [6.1. Предсказуемое потребление ресурсов](#61-----------------------------------)
    + [6.2. Реакция на OOM и throttling](#62------------oom---throttling)
  * [7. Логирование и наблюдаемость](#7----------------------------)
    + [7.1. Логи в stdout/stderr](#71--------stdout-stderr)
    + [7.2. Метрики и трассировка](#72----------------------)
  * [8. Kubernetes-native подход](#8-kubernetes-native-------)
    + [8.1. Stateless-архитектура](#81-stateless------------)
    + [8.2. Разделение ответственности](#82---------------------------)
  * [9. Методики переноса существующих Docker-контейнеров](#9--------------------------------docker------------)
- [Сопоставление сущностей Docker Compose и Kubernetes](#------------------------docker-compose---kubernetes)
- [Адаптация Docker Compose (Redmine + MySQL) для Kubernetes](#----------docker-compose--redmine---mysql------kubernetes)
  * [1. Исходный docker-compose.yml:](#1----------docker-composeyml-)
  * [2. Неявные допущения Docker Compose](#2-------------------docker-compose)
  * [3. Декомпозиция Docker Compose на Kubernetes-ресурсы](#3--------------docker-compose----kubernetes--------)
  * [4. Адаптация MySQL (db)](#4-----------mysql--db-)
    + [4.1. Stateful-характер сервиса `StatefulSet`.](#41-stateful-------------------statefulset-)
    + [4.2. Хранилище данных](#42-----------------)
    + [4.3. Конфигурация и секреты](#43-----------------------)
    + [4.4. Сетевое взаимодействие](#44-----------------------)
    + [4.5. Порты](#45------)
  * [5. Адаптация Redmine](#5-----------redmine)
    + [5.1. Тип нагрузки](#51-------------)
    + [5.2. Конфигурация приложения](#52------------------------)
    + [5.3. Подключение к базе данных](#53--------------------------)
    + [5.4. Порты и доступ извне](#54---------------------)
  * [6. Управление жизненным циклом](#6----------------------------)
    + [6.1. Readiness Probe](#61-readiness-probe)
    + [6.2. Liveness Probe](#62-liveness-probe)
    + [6.3. Корректное завершение](#63----------------------)
  * [7. Управление ресурсами](#7---------------------)
    + [7.1. Ограничения ресурсов](#71---------------------)
  * [8. Логирование](#8------------)
    + [8.1. Redmine](#81-redmine)
    + [8.2. MySQL](#82-mysql)
  * [9. Итоговая архитектура в Kubernetes](#9------------------------kubernetes)
    + [9.1. Набор ресурсов](#91---------------)
- [Итоговые манифесты Kubernetes для Redmine](#-------------------kubernetes-----redmine)
  * [1. Namespace (опционально)](#1-namespace--------------)
  * [2. ConfigMap для MySQL и Redmine](#2-configmap-----mysql---redmine)
  * [3. Secrets](#3-secrets)
  * [4. PersistentVolumeClaim для MySQL](#4-persistentvolumeclaim-----mysql)
  * [5. Service для MySQL](#5-service-----mysql)
  * [6. StatefulSet для MySQL](#6-statefulset-----mysql)
  * [7. Service для Redmine](#7-service-----redmine)
  * [8. Deployment для Redmine](#8-deployment-----redmine)
  * [9.  Ingress для внешнего доступа (Опционально)](#9--ingress-----------------------------------)
  * [10. Улучшения для Production](#10---------------production)
    + [10.1 initContainer (ожидание MySQL) добавить в Deployment Redmine (Kubernetes-native замена `depends_on`.)](#101-initcontainer-----------mysql-------------deployment-redmine--kubernetes-native---------depends-on--)
    + [10.2 Добавление TLS в Ingress](#102------------tls---ingress)
    + [10.3 Пробы для MySQL](#103-----------mysql)
    + [10.4 HPA для Redmine](#104-hpa-----redmine)
    + [10.5 PodSecurityContext / non-root](#105-podsecuritycontext---non-root)
- [Итоговая структура чарта](#------------------------)
  * [Как развернуть проект локально.](#------------------------------)
- [BONUS Конвертация Docker-compose с помощью приложения Kompose](#bonus-------------docker-compose----------------------kompose)
