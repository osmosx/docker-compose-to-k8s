...
# Оглавление
  - [1. Адаптация контейнерного образа (Dockerfile-level)](#1-Адаптация-контейнерного-образа-Dockerfile-level)
    - [1.1. Один процесс — один контейнер](#11-Один-процесс-—-один-контейнер)
    - [1.2. Корректная работа PID 1](#12-Корректная-работа-PID-1)
    - [1.3. Отказ от root-пользователя](#13-Отказ-от-root-пользователя)
    - [1.4 Для надежного развертывания и отката в Kubernetes используется концепция неизменяемых развертываний.](#14-Для-надежного-развертывания-и-отката-в-Kubernetes-используется-концепция-неизменяемых-развертываний)
  - [2. Конфигурация и переменные окружения](#2-Конфигурация-и-переменные-окружения)
    - [2.1. Декларативная конфигурация](#21-Декларативная-конфигурация)
    - [2.2. Использование ConfigMap и Secret](#22-Использование-ConfigMap-и-Secret)
  - [3. Работа с файловой системой и данными](#3-Работа-с-файловой-системой-и-данными)
    - [3.1. Эфемерность контейнера](#31-Эфемерность-контейнера)
    - [3.2. Volume-ориентированная архитектура](#32-Volume-ориентированная-архитектура)
  - [4. Сетевое взаимодействие](#4-Сетевое-взаимодействие)
    - [4.1. Отказ от жёсткой привязки к IP](#41-Отказ-от-жёсткой-привязки-к-IP)
    - [4.2. Один порт — один сервис](#42-Один-порт-—-один-сервис)
  - [5. Health-checks и жизненный цикл](#5-Health-checks-и-жизненный-цикл)
    - [5.1. Readiness и Liveness probes](#51-Readiness-и-Liveness-probes)
    - [5.2. Startup-поведение](#52-Startup-поведение)
  - [6. Управление ресурсами](#6-Управление-ресурсами)
    - [6.1. Предсказуемое потребление ресурсов](#61-Предсказуемое-потребление-ресурсов)
    - [6.2. Реакция на OOM и throttling](#62-Реакция-на-OOM-и-throttling)
  - [7. Логирование и наблюдаемость](#7-Логирование-и-наблюдаемость)
    - [7.1. Логи в stdout/stderr](#71-Логи-в-stdoutstderr)
    - [7.2. Метрики и трассировка](#72-Метрики-и-трассировка)
  - [8. Kubernetes-native подход](#8-Kubernetes-native-подход)
    - [8.1. Stateless-архитектура](#81-Stateless-архитектура)
    - [8.2. Разделение ответственности](#82-Разделение-ответственности)
  - [9. Методики переноса существующих Docker-контейнеров](#9-Методики-переноса-существующих-Docker-контейнеров)
- [Сопоставление сущностей Docker Compose и Kubernetes](#Сопоставление-сущностей-Docker-Compose-и-Kubernetes)
- [Адаптация Docker Compose (Redmine + MySQL) для Kubernetes](#Адаптация-Docker-Compose-Redmine--MySQL-для-Kubernetes)
  - [1. Исходный docker-compose.yml:](#1-Исходный-docker-composeyml)
  - [2. Неявные допущения Docker Compose](#2-Неявные-допущения-Docker-Compose)
  - [3. Декомпозиция Docker Compose на Kubernetes-ресурсы](#3-Декомпозиция-Docker-Compose-на-Kubernetes-ресурсы)
  - [4. Адаптация MySQL (db)](#4-Адаптация-MySQL-db)
    - [4.1. Stateful-характер сервиса `StatefulSet`.](#41-Stateful-характер-сервиса-StatefulSet)
    - [4.2. Хранилище данных](#42-Хранилище-данных)
    - [4.3. Конфигурация и секреты](#43-Конфигурация-и-секреты)
    - [4.4. Сетевое взаимодействие](#44-Сетевое-взаимодействие)
    - [4.5. Порты](#45-Порты)
  - [5. Адаптация Redmine](#5-Адаптация-Redmine)
    - [5.1. Тип нагрузки](#51-Тип-нагрузки)
    - [5.2. Конфигурация приложения](#52-Конфигурация-приложения)
    - [5.3. Подключение к базе данных](#53-Подключение-к-базе-данных)
    - [5.4. Порты и доступ извне](#54-Порты-и-доступ-извне)
  - [6. Управление жизненным циклом](#6-Управление-жизненным-циклом)
    - [6.1. Readiness Probe](#61-Readiness-Probe)
    - [6.2. Liveness Probe](#62-Liveness-Probe)
    - [6.3. Корректное завершение](#63-Корректное-завершение)
  - [7. Управление ресурсами](#7-Управление-ресурсами)
    - [7.1. Ограничения ресурсов](#71-Ограничения-ресурсов)
  - [8. Логирование](#8-Логирование)
    - [8.1. Redmine](#81-Redmine)
    - [8.2. MySQL](#82-MySQL)
  - [9. Итоговая архитектура в Kubernetes](#9-Итоговая-архитектура-в-Kubernetes)
    - [9.1. Набор ресурсов](#91-Набор-ресурсов)
- [Итоговые манифесты Kubernetes для Redmine](#Итоговые-манифесты-Kubernetes-для-Redmine)
  - [1. Namespace (опционально)](#1-Namespace-опционально)
  - [2. ConfigMap для MySQL и Redmine](#2-ConfigMap-для-MySQL-и-Redmine)
  - [3. Secrets](#3-Secrets)
  - [4. PersistentVolumeClaim для MySQL](#4-PersistentVolumeClaim-для-MySQL)
  - [5. Service для MySQL](#5-Service-для-MySQL)
  - [6. StatefulSet для MySQL](#6-StatefulSet-для-MySQL)
  - [7. Service для Redmine](#7-Service-для-Redmine)
  - [8. Deployment для Redmine](#8-Deployment-для-Redmine)
  - [9.  Ingress для внешнего доступа (Опционально)](#9--Ingress-для-внешнего-доступа-Опционально)
  - [10. Улучшения для Production](#10-Улучшения-для-Production)
    - [10.1 initContainer (ожидание MySQL) добавить в Deployment Redmine (Kubernetes-native замена `depends_on`.)](#101-initContainer-ожидание-MySQL-добавить-в-Deployment-Redmine-Kubernetes-native-замена depends_on)
    - [10.2 Добавление TLS в Ingress](#102-Добавление-TLS-в-Ingress)
    - [10.3 Пробы для MySQL](#103-Пробы-для-MySQL)
    - [10.4 HPA для Redmine](#104-HPA-для-Redmine)
    - [10.5 PodSecurityContext / non-root](#105-PodSecurityContext--non-root)
- [Итоговая структура чарта](#Итоговая-структура-чарта)
  - [Как развернуть проект локально.](#Как-развернуть-проект-локально)
- [BONUS Конвертация Docker-compose с помощью приложения Kompose](#BONUS-Конвертация-Docker-compose-с-помощью-приложения-Kompose)
  - [Исходный docker-compose.yml](#Исходный-docker-composeyml)
  - [Подход к конвертации](#Подход-к-конвертации)
  - [Подготовленный docker-compose.yml](#Подготовленный-docker-composeyml)
  - [Установка Kompose](#Установка-Kompose)
  - [Конвертация в Helm Chart](#Конвертация-в-Helm-Chart)
  - [Полученная структура Helm Chart](#Полученная-структура-Helm-Chart)
  - [Проверка Helm Chart](#Проверка-Helm-Chart)
  - [Развертывание в Kubernetes](#Развертывание-в-Kubernetes)
  - [Проверка ресурсов](#Проверка-ресурсов)
  - [Итог](#Итог)
